...
| orchestration versus protocol implementation | PARTIALLY COMPLETE | `bw_GetStatus_DoWork` now orchestrates multiple seams (HTTP request/retry/redirect/interpretation, Cloudflare bypass interpretation, FTP request/status mapping, ping retry, identity shaping, terminal finalization, SSL property mapping), but still contains substantial protocol-specific side-effect blocks (response stream persistence/meta extraction, share lookup, SSL certificate acquisition, bypass invocation). |
