...
| WHOIS workflow (`GetWhoIsInfo`) | PARTIALLY COMPLETE | UI payload/tab transition shaping now uses deterministic seam (`EndpointWhoIsPresentationBuilder` with deterministic tests), while cancellation/lifecycle characterization remains broader than this workflow-only split. |
