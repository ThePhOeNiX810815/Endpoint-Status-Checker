...
| HTTP response metadata extraction | PARTIALLY COMPLETE | Success/error interpretation seam extracted in `src/EndpointHttpResponseInterpreter.cs`; HTTP response body stream/meta gating extracted into `src/EndpointHttpResponseBodyProcessor.cs` and covered in `tests/HttpCompatibility.Tests/EndpointHttpRequestFactoryTests.cs`. Remaining header-to-property collection and HTML metadata parsing side effects (`ResolvePageMetaInfo`) remain form-owned orchestration. |
...
| orchestration versus protocol implementation | PARTIALLY COMPLETE | `bw_GetStatus_DoWork` now orchestrates multiple seams (HTTP request/retry/redirect/interpretation, HTTP response body stream/meta gating, Cloudflare bypass interpretation, FTP request/status mapping, ping retry, identity shaping, share-result shaping, terminal finalization, SSL property mapping), but still contains substantial protocol-specific side-effect blocks (HTML metadata parsing side effects, share acquisition, SSL certificate acquisition, bypass invocation). |
