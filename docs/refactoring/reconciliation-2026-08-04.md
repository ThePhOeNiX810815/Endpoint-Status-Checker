...
| UI invoke network isolation | `src/EndpointDetailsDialog.cs` | `GetIPGeoInfo`, `GetWhoIsInfo`, `GetMACVendor`, `GetWebsiteFavicon`, `ValidatePageLinks` | PARTIALLY COMPLETE | Geolocation and WHOIS compute happen off invoke; MAC/favicon/link-validation still perform heavy work without cohesive cancellation/transition boundaries. | Geo: `EndpointGeoLocationPresentationBuilder`; WHOIS: `EndpointWhoIsPresentationBuilder` | `EndpointGeoLocationPresentationBuilderTests`, `EndpointWhoIsPresentationBuilderTests` | Uneven workflow isolation and missing deterministic boundaries in remaining paths. | No confirmed network I/O currently runs inside `ThreadSafeInvoke`, but several workflows still lack robust boundary extraction and cancellation characterization. |
| VirusTotal scan submission | `src/EndpointDetailsDialog.cs` | `GetVirusTotalScanReport` | PARTIALLY COMPLETE | Iterative retry and cancellation are in place; status-label updates remain in dialog apply blocks. | `EndpointVirusTotalScanRetryExecutor`, `EndpointVirusTotalCancellationGate` | `EndpointVirusTotalScanRetryExecutorTests`, `EndpointVirusTotalReportWorkflowTests` | Dialog-level lifecycle sequencing is not fully characterized end-to-end. | Highest-risk recursion/sleep/retry path was extracted; remaining risk is orchestration sequencing under disposal/timing races. |
| VirusTotal report polling | `src/EndpointDetailsDialog.cs` | `TIMER_VirusTotalResult_Tick`, `BW_VirusTotal_Report_DoWork` | PARTIALLY COMPLETE | Timer gate checks cancellation and worker-busy state; report transition mapping is deterministic; catch blocks still swallow exceptions without explicit terminal transition. | `EndpointVirusTotalReportWorkflow`, `EndpointVirusTotalCancellationGate`, `EndpointTaskSyncBridge` | `EndpointVirusTotalReportWorkflowTests`, `EndpointTaskSyncBridgeTests` | Timer/worker lifecycle still relies on form-level mutable state and swallowed failures. | Transition shaping and cancellation decision seams exist, but broader lifecycle guarantees are incomplete. |
| MAC vendor lookup | `src/EndpointDetailsDialog.cs` | `GetMACVendor`, `GetImageFromURL` | PARTIALLY COMPLETE | Multi-call lookup remains network-bound but now separates presentation/link shaping and applies UI state only inside invoke block. | `EndpointMacVendorPresentationBuilder`, `EndpointDetailsDialogCancellationGate` | `tests/EndpointCheckingCore.Tests/EndpointDetailsDialogWorkflowSeamsTests.cs` | Nested exception swallowing and workflow-level lifecycle transitions are still only partially characterized. | Deterministic result-shaping and cancellation decision seams now exist; broader transition/error-state characterization remains pending. |
| Favicon/image retrieval | `src/EndpointDetailsDialog.cs` | `GetWebsiteFavicon`, `GetImageFromURL`, `GetImageFromGoogleMapsAPI`, `GetCountryFlagByCode` | PARTIALLY COMPLETE | Favicon retrieval now executes non-recursive planned attempts and applies image on UI invoke; geo/map/flag retrieval remains background-only. | `EndpointFaviconLookupPlanBuilder`, `EndpointDetailsDialogCancellationGate`, `EndpointGeoLocationPresentationBuilder` | `EndpointDetailsDialogWorkflowSeamsTests`, `EndpointGeoLocationPresentationBuilderTests` | Generic image helper workflows still rely on swallowed exceptions and lack full transition characterization. | High-risk recursive retry and background-thread UI mutation were removed; remaining risk is broader image-workflow lifecycle consistency. |
| Link-validation loop | `src/EndpointDetailsDialog.cs` | `ValidatePageLinks` | NOT STARTED | Runs long network loop on background thread with `Application.DoEvents()` on each iteration and no explicit cancellation/disposal gate. | None | None | Reentrancy/timing risk and no deterministic transition/cancellation seam. | This remains one of the highest-risk dialog async-safety workflows. |
| Ping live refresh timer | `src/EndpointDetailsDialog.cs` | `TIMER_PingRefresh_Tick`, `DoPing`, `SetPingResponse` | PARTIALLY COMPLETE | Timer triggers background ping; UI apply is invoke-wrapped; no explicit disposal/cancellation gate before scheduling/apply. | None | None in dialog-specific suite | Timer can enqueue work near close; safety currently relies on invoke wrapper swallow behavior. | Behavior is compatible but not deterministically characterized for close/dispose races. |
...
| Wrapper-level `DoEvents` usage | `src/EndpointDetailsDialog.cs`, `src/UiThreadHelpers.cs` | `NewBackgroundThread`, `ThreadSafeInvoke`, `UiThreadHelpers.StartBackgroundThread`, `UiThreadHelpers.SafeInvoke` | PARTIALLY COMPLETE | Dialog wrappers still pass `Application.DoEvents` into helper callbacks. | `UiThreadHelpers` | `tests/EndpointCheckingCore.Tests/UiThreadHelpersTests.cs` | Reentrancy/ordering risk is only partially characterized and still present. | Consolidation is complete but risk reduction is deferred into Priority 5 inventory/remediation. |
| Form closing/disposal lifecycle | `src/EndpointDetailsDialog.cs` | `EndpointDetailsDialog_FormClosing` | PARTIALLY COMPLETE | Sets `virusTotalScanCancelled`, disposes secondary form reference, and runs GC wait; no centralized cancellation contract for all active workflows/timers. | Partial via `EndpointVirusTotalCancellationGate` (VirusTotal-only) | `EndpointVirusTotalReportWorkflowTests` (gate decision only) | Non-VirusTotal workflows can still outlive close boundary. | Closing safety is improved for VirusTotal but incomplete dialog-wide. |

Priority 4 remains `in progress`. The highest-risk unresolved workflows in live code are `GetMACVendor`, `GetWebsiteFavicon` recursion/fallback, and `ValidatePageLinks` loop lifecycle/cancellation behavior.
